...
static void handleArisenRicardianAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  ...
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
   ...
   D->addAttr(::new (S.Context)
                 ArisenRicardianAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenNotifyAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
   ...
   D->addAttr(::new (S.Context)
                 ArisenNotifyAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenContractAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
    return;
  D->addAttr(::new (S.Context)
                 ArisenContractAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenABIAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 ArisenWasmABIAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenWasmActionAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 ArisenWasmActionAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenWasmNotifyAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 ArisenWasmNotifyAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenActionAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 ArisenActionAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleArisenTableAttribute(Sema &S, Decl *D, const AttributeList &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str, Replacement;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 ArisenTableAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}
...
static void ProcessDeclAttribute(Sema &S, Scope *scope, Decl *D,
  ...
  default:
    ...
    break;
  case AttributeList::AT_ArisenWasmImport:
    handleSimpleAttribute<ArisenWasmImportAttr>(S, D, AL);
    break;
  case AttributeList::AT_ArisenWasmEntry:
    handleSimpleAttribute<ArisenWasmEntryAttr>(S, D, AL);
    break;
  case AttributeList::AT_ArisenIgnore:
    handleSimpleAttribute<ArisenIgnoreAttr>(S, D, AL);
    break;
  case AttributeList::AT_ArisenAction:
    handleArisenActionAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenTable:
    handleArisenTableAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenWasmABI:
    handleArisenABIAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenWasmAction:
    handleArisenWasmActionAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenWasmNotify:
    handleArisenWasmNotifyAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenContract:
    handleArisenContractAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenRicardian:
    handleArisenRicardianAttribute(S, D, AL);
    break;
  case AttributeList::AT_ArisenNotify:
    handleArisenNotifyAttribute(S, D, AL);
    break;
