...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
     if (FD->hasAttr<ArisenWasmImportAttr>())
        isWasmImport = true;
     if (FD->hasAttr<ArisenWasmEntryAttr>())
        isWasmEntry = true;
     if (FD->hasAttr<ArisenWasmABIAttr>())
        isWasmABI = true;
     if (FD->hasAttr<ArisenWasmActionAttr>())
        isWasmAction = true;
     if (FD->hasAttr<ArisenWasmNotifyAttr>())
        isWasmNotify = true;
  ...
  if (isWasmImport)
   F->addFnAttr("arisen_wasm_import", "true");

  if (isWasmEntry)
   F->addFnAttr("arisen_wasm_entry", "true");
     
  if (isWasmABI)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("arisen_wasm_abi", FD->getWasmABI().c_str());
     }
  if (isWasmAction)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("arisen_wasm_action", FD->getArisenWasmAction().c_str());
     }
  if (isWasmNotify)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("arisen_wasm_notify", FD->getArisenWasmNotify().c_str());
     }
